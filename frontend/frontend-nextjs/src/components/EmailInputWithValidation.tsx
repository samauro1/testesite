...
const EmailInputWithValidation: React.FC<EmailInputWithValidationProps> = ({ 
  value, 
  onChange, 
  onDuplicateConfirm,
  placeholder, 
  ...
  const checkEmailDuplicate = async (email: string) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:3001/api/pacientes?search=${encodeURIComponent(email)}&limit=1`, {
        headers: {
      ...
      if (response.ok) {
        ...
        // Verificar se algum paciente tem este email
        const duplicatePatient = pacientes.find((p: any) => p.email === email);
  ...
  const handleConfirmDuplicate = (confirmed: boolean) => {
    ...
    if (!confirmed) {
      // Limpar o email se não confirmar
      onChange('');
    }
  };
          ...
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            ...
            <div className="mb-4">
              <p className="text-gray-600 mb-3">
                Este email já está associado a outro paciente:
              </p>
              ...
              <div className="bg-gray-50 rounded-lg p-3">
                ...
                <p className="text-sm text-gray-600">Email: {duplicateInfo.email}</p>
              </div>
              
              <p className="text-gray-600 mt-3">
                Deseja usar este email mesmo assim? Uma observação será adicionada automaticamente.
              </p>
            </div>
