  ...
  switch (tipo) {
    ...
    case 'bpa2':
      ...
      `, [
        avaliacaoId,
        'Concentrada',
        dados.acertos_concentrada || dados.acertos_sustentada || 0,
        dados.erros_concentrada || dados.erros_sustentada || 0,
        dados.omissoes_concentrada || dados.omissoes_sustentada || 0,
        resultado.concentrada.pontos,
        resultado.concentrada.percentil,
        resultado.concentrada.classificacao
      ]);
      ...
      `, [
        avaliacaoId,
        'Geral',
        0, // Não há acertos/erros/omissões para Atenção Geral (é soma dos pontos)
        0,
        0,
        resultado.geral.pontos, // Soma: AA + AC + AD
...
async function calcularBPA2(tabelaId, dados) {
  ...
  // Aceitar tanto nomenclatura antiga (sustentada) quanto nova (concentrada)
  const { 
    acertos_sustentada, erros_sustentada, omissoes_sustentada,
    acertos_concentrada, erros_concentrada, omissoes_concentrada,
  ...
  const acertos_c = Number(acertos_concentrada || acertos_sustentada) || 0;
  const erros_c = Number(erros_concentrada || erros_sustentada) || 0;
  const omissoes_c = Number(omissoes_concentrada || omissoes_sustentada) || 0;
  ...
  const pontos_concentrada = acertos_c - erros_c - omissoes_c;
  const pontos_alternada = acertos_a - erros_a - omissoes_a;
  const pontos_dividida = acertos_d - erros_d - omissoes_d;
  
  console.log('🧮 BPA2: Pontos calculados', { pontos_alternada, pontos_concentrada, pontos_dividida });
  ...
  const resultConcentrada = await query(`
    SELECT percentil, classificacao 
    FROM normas_bpa2 
    WHERE tabela_id = $1 AND tipo_atencao = 'Concentrada' 
    AND $2 BETWEEN pontos_min AND pontos_max
  `, [tabelaId, pontos_concentrada]);
  ...
  // Buscar percentil e classificação na tabela normativa para Atenção Geral
  const resultGeral = await query(`
    SELECT percentil, classificacao 
    FROM normas_bpa2 
    WHERE tabela_id = $1 AND tipo_atencao = 'Geral' 
    AND $2 BETWEEN pontos_min AND pontos_max
  `, [tabelaId, pontos_geral]);
