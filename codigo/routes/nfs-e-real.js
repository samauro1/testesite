...
const NfsEServiceReal = require('../utils/nfsEServiceReal');
const { pool } = require('../config/database');

// Configuração do serviço real
const nfsEServiceReal = new NfsEServiceReal({
  certificadoPath: process.env.CERTIFICADO_PATH || './certificado.pfx',
  certificadoSenha: process.env.CERTIFICADO_SENHA || '',
...
router.post('/testar-conexao', async (req, res) => {
  try {
    console.log('🧪 POST /testar-conexao - Prefeitura SP');
    
    const resultado = await nfsEServiceReal.testarConexao();
...
router.post('/emitir-real', async (req, res) => {
  try {
    console.log('🧾 POST /emitir-real - NFS-e Real');
    ...
    const resultado = await nfsEServiceReal.emitirNfsE(dadosRPS);
