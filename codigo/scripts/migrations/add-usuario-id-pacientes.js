...
async function migrate() {
  ...
  try {
    console.log('🔄 Iniciando migração: Adicionar usuario_id à tabela pacientes...\n');
    ...
    // 2. Buscar o primeiro usuário administrador para atribuir aos pacientes existentes
    ...
    // 3. Atribuir todos os pacientes existentes ao administrador
    console.log('\n3️⃣  Atribuindo pacientes existentes ao administrador...');
    const updateResult = await client.query(`
      UPDATE pacientes 
      SET usuario_id = $1 
      WHERE usuario_id IS NULL
    `, [adminId]);
    console.log(`✅ ${updateResult.rowCount} pacientes atribuídos ao administrador`);
    ...
    console.log('\n📊 Resumo:');
    console.log(`   - Coluna usuario_id adicionada à tabela pacientes`);
    console.log(`   - ${updateResult.rowCount} pacientes existentes atribuídos ao admin`);
