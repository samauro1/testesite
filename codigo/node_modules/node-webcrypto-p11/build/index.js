...
class EcCrypto {
    ...
    spki2jwk(raw) {
        ...
        const namedCurveId = asn1Schema__namespace.AsnParser.parse(keyInfo.publicKeyAlgorithm.parameters, core__namespace.asn1.ObjectIdentifier);
        const namedCurve = core__namespace.EcCurves.get(namedCurveId.value);
        const ecPublicKey = new core__namespace.asn1.EcPublicKey(keyInfo.publicKey);
        const json = jsonSchema__namespace.JsonSerializer.toJSON(ecPublicKey);
        return {
            kty: "EC",
            crv: namedCurve.name,
            ...json,
    ...
    jwk2pkcs(jwk) {
        ...
        const namedCurve = core__namespace.EcCurves.get(jwk.crv);
        const ecPrivateKey = jsonSchema__namespace.JsonParser.fromJSON(jwk, { targetSchema: core__namespace.asn1.EcPrivateKey });
        const keyInfo = new core__namespace.asn1.PrivateKeyInfo();
        keyInfo.privateKeyAlgorithm = new core__namespace.asn1.AlgorithmIdentifier();
        keyInfo.privateKeyAlgorithm.algorithm = id_ecPublicKey;
        keyInfo.privateKeyAlgorithm.parameters = namedCurve.raw;
        keyInfo.privateKey = asn1Schema__namespace.AsnSerializer.serialize(ecPrivateKey);
        return asn1Schema__namespace.AsnSerializer.serialize(keyInfo);
    }
    getCoordinate(b64, coordinateLength) {
        const buf = pvtsutils__namespace.Convert.FromBase64Url(b64);
    ...
    jwk2spki(jwk) {
        ...
        const namedCurve = core__namespace.EcCurves.get(jwk.crv);
        const ecPublicKey = jsonSchema__namespace.JsonParser.fromJSON(jwk, { targetSchema: core__namespace.asn1.EcPublicKey });
    ...
    pkcs2jwk(raw) {
        const keyInfo = asn1Schema__namespace.AsnParser.parse(raw, core__namespace.asn1.PrivateKeyInfo);
        ...
        const namedCurveId = asn1Schema__namespace.AsnParser.parse(keyInfo.privateKeyAlgorithm.parameters, core__namespace.asn1.ObjectIdentifier);
        const namedCurve = core__namespace.EcCurves.get(namedCurveId.value);
        const ecPrivateKey = asn1Schema__namespace.AsnParser.parse(keyInfo.privateKey, core__namespace.asn1.EcPrivateKey);
        const json = jsonSchema__namespace.JsonSerializer.toJSON(ecPrivateKey);
        return {
            kty: "EC",
            crv: namedCurve.name,
            ...json,
    ...
    async onDeriveBits(algorithm, baseKey, length) {
        return new Promise((resolve, reject) => {
            ...
            const template = {
                token: false,
                sensitive: false,
                class: graphene__namespace.ObjectClass.SECRET_KEY,
                keyType: graphene__namespace.KeyType.GENERIC_SECRET,
                extractable: true,
                encrypt: true,
                decrypt: true,
                valueLen: valueLen >> 3,
            ...
            this.container.session.deriveKey({
                name: "ECDH1_DERIVE",
                params: new graphene__namespace.EcdhParams(graphene__namespace.EcKdf.NULL, null, ecPoint),
    ...
    wc2pk11(alg, _keyAlg) {
        ...
        switch (hashAlg) {
            ...
            case "SHA-512":
                ...
                break;
            default:
                throw new core__namespace.OperationError(`Cannot create PKCS11 mechanism from algorithm '${hashAlg}'`);
        }
...
class HmacProvider extends core__namespace.HmacProvider {
    ...
    async onGenerateKey(algorithm, extractable, keyUsages) {
        return new Promise((resolve, reject) => {
            ...
            this.container.session.generateKey(graphene__namespace.KeyGenMechanism.GENERIC_SECRET, template, (err, key) => {
                try {
                    if (err) {
                        reject(new core__namespace.CryptoError(`HMAC: Cannot generate new key\n${err.message}`));
                    }
                    else {
                        if (!key) {
                            throw new Error("Cannot get key from callback function");
    ...
    createTemplate(params) {
        ...
        template.keyType = graphene__namespace.KeyType.GENERIC_SECRET;
        return template;
    }
    wc2pk11(alg, keyAlg) {
        let res;
        switch (keyAlg.hash.name.toUpperCase()) {
            ...
            case "SHA-512":
                ...
                break;
            default:
                throw new core__namespace.OperationError(`Cannot create PKCS11 mechanism from algorithm '${keyAlg.hash.name}'`);
        }
...
class RsaCrypto {
    ...
    async generateKey(algorithm, extractable, keyUsages) {
        ...
        return new Promise((resolve, reject) => {
            this.container.session.generateKeyPair(graphene__namespace.KeyGenMechanism.RSA, publicTemplate, privateTemplate, (err, keys) => {
                try {
                    if (err) {
                        reject(new core__namespace.CryptoError(`Rsa: Can not generate new key\n${err.message}`));
                    }
                    else {
                        if (!keys) {
                            throw new Error("Cannot get keys from callback function");
    ...
    async exportKey(format, key) {
        switch (format.toLowerCase()) {
            ...
            case "raw": {
                ...
                const asn = asn1Schema__namespace.AsnConvert.parse(spki, core__namespace.asn1.PublicKeyInfo);
                return asn.publicKey;
            }
            default:
                throw new core__namespace.OperationError("format: Must be 'raw', 'jwk', 'pkcs8' or 'spki'");
        }
    }
    async importKey(format, keyData, algorithm, extractable, keyUsages) {
        switch (format.toLowerCase()) {
    ...
    async exportJwkPublicKey(key) {
        ...
        const jwk = {
            ...
            ext: true,
            key_ops: key.usages,
            e: pvtsutils__namespace.Convert.ToBase64Url(pkey.publicExponent),
            n: pvtsutils__namespace.Convert.ToBase64Url(pkey.modulus),
        };
        return jwk;
    }
    async exportJwkPrivateKey(key) {
        ...
        const jwk = {
            ...
            e: pvtsutils__namespace.Convert.ToBase64Url(pkey.publicExponent),
            n: pvtsutils__namespace.Convert.ToBase64Url(pkey.modulus),
            d: pvtsutils__namespace.Convert.ToBase64Url(pkey.privateExponent),
            p: pvtsutils__namespace.Convert.ToBase64Url(pkey.prime1),
            q: pvtsutils__namespace.Convert.ToBase64Url(pkey.prime2),
            dp: pvtsutils__namespace.Convert.ToBase64Url(pkey.exp1),
            dq: pvtsutils__namespace.Convert.ToBase64Url(pkey.exp2),
            qi: pvtsutils__namespace.Convert.ToBase64Url(pkey.coefficient),
        };
        return jwk;
    }
    importJwkPrivateKey(jwk, algorithm, extractable, keyUsages) {
    ...
    createTemplate(params) {
        ...
        template.keyType = graphene__namespace.KeyType.RSA;
        return template;
    }
    jwk2spki(jwk) {
        const key = jsonSchema__namespace.JsonParser.fromJSON(jwk, { targetSchema: core__namespace.asn1.RsaPublicKey });
        const keyInfo = new core__namespace.asn1.PublicKeyInfo();
        keyInfo.publicKeyAlgorithm.algorithm = "1.2.840.113549.1.1.1";
        keyInfo.publicKeyAlgorithm.parameters = null;
        keyInfo.publicKey = asn1Schema__namespace.AsnSerializer.serialize(key);
        return asn1Schema__namespace.AsnSerializer.serialize(keyInfo);
    }
    jwk2pkcs(jwk) {
        const key = jsonSchema__namespace.JsonParser.fromJSON(jwk, { targetSchema: core__namespace.asn1.RsaPrivateKey });
        const keyInfo = new core__namespace.asn1.PrivateKeyInfo();
        keyInfo.privateKeyAlgorithm.algorithm = "1.2.840.113549.1.1.1";
        keyInfo.privateKeyAlgorithm.parameters = null;
        keyInfo.privateKey = asn1Schema__namespace.AsnSerializer.serialize(key);
        return asn1Schema__namespace.AsnSerializer.serialize(keyInfo);
    }
    pkcs2jwk(raw) {
        const keyInfo = asn1Schema__namespace.AsnParser.parse(raw, core__namespace.asn1.PrivateKeyInfo);
        ...
        const key = asn1Schema__namespace.AsnParser.parse(keyInfo.privateKey, core__namespace.asn1.RsaPrivateKey);
        const json = jsonSchema__namespace.JsonSerializer.toJSON(key);
        return {
            kty: "RSA",
            ...json,
        };
    ...
    spki2jwk(raw) {
        ...
        const key = asn1Schema__namespace.AsnParser.parse(keyInfo.publicKey, core__namespace.asn1.RsaPublicKey);
        const json = jsonSchema__namespace.JsonSerializer.toJSON(key);
        return {
            kty: "RSA",
            ...json,
    ...
    wc2pk11(alg, keyAlg) {
        ...
        switch (keyAlg.hash.name.toUpperCase()) {
            case "SHA-1":
                ...
                param = new graphene__namespace.RsaPssParams(graphene__namespace.MechanismEnum.SHA1, graphene__namespace.RsaMgf.MGF1_SHA1, saltLen);
                break;
            case "SHA-224":
                mech = "SHA224_RSA_PKCS_PSS";
                param = new graphene__namespace.RsaPssParams(graphene__namespace.MechanismEnum.SHA224, graphene__namespace.RsaMgf.MGF1_SHA224, saltLen);
                break;
            case "SHA-256":
                mech = "SHA256_RSA_PKCS_PSS";
                param = new graphene__namespace.RsaPssParams(graphene__namespace.MechanismEnum.SHA256, graphene__namespace.RsaMgf.MGF1_SHA256, saltLen);
                break;
            case "SHA-384":
                mech = "SHA384_RSA_PKCS_PSS";
                param = new graphene__namespace.RsaPssParams(graphene__namespace.MechanismEnum.SHA384, graphene__namespace.RsaMgf.MGF1_SHA384, saltLen);
                break;
            case "SHA-512":
                mech = "SHA512_RSA_PKCS_PSS";
                param = new graphene__namespace.RsaPssParams(graphene__namespace.MechanismEnum.SHA512, graphene__namespace.RsaMgf.MGF1_SHA512, saltLen);
                break;
            default:
                throw new core__namespace.OperationError(`Cannot create PKCS11 mechanism from algorithm '${keyAlg.hash.name}'`);
        }
...
class RsaOaepProvider extends core__namespace.RsaOaepProvider {
    ...
    hasRsaPkcsMechanism() {
        ...
        for (let i = 0; i < mechanisms.length; i++) {
            const mechanism = mechanisms.tryGetItem(i);
            if (mechanism && (mechanism.type === graphene__namespace.MechanismEnum.RSA_X_509)) {
                return true;
            }
        }
        return false;
    ...
    async onEncryptRsaX509(algorithm, key, data) {
        ...
        crypto__namespace.randomFillSync(seed);
        const dataBlock = message.subarray(hashSize + 1);
        const labelHash = crypto__namespace.createHash(key.algorithm.hash.name.replace("-", ""))
        ...
        return new Promise((resolve, reject) => {
            ...
            const context = Buffer.alloc((key.algorithm).modulusLength >> 3);
            this.container.session.createCipher(graphene__namespace.MechanismEnum.RSA_X_509, key.key)
                .once(buf, context, (err, data2) => {
                if (err) {
                    reject(err);
                }
                else {
                    resolve(core__namespace.BufferSourceConverter.toArrayBuffer(data2));
                }
            });
        });
    }
    ...
    async onDecryptRsaX509(algorithm, key, data) {
        ...
        const buf = Buffer.from(data);
        const context = Buffer.alloc((key.algorithm).modulusLength >> 3);
        const decipher = this.container.session.createDecipher(graphene__namespace.MechanismEnum.RSA_X_509, key.key);
        try {
            await alwaysAuthenticate(key, this.container, "decrypt");
        }
        catch (e) {
        ...
        if (!core__namespace.BufferSourceConverter.isEqual(expectedLabelHash, labelHash)) {
        ...
        return core__namespace.BufferSourceConverter.toArrayBuffer(dataBlock.subarray(index));
    }
    async onExportKey(format, key) {
        return this.crypto.exportKey(format, key);
    }
    ...
    wc2pk11(alg, keyAlg) {
        ...
        switch (keyAlg.hash.name.toUpperCase()) {
            case "SHA-1":
                params = new graphene__namespace.RsaOaepParams(graphene__namespace.MechanismEnum.SHA1, graphene__namespace.RsaMgf.MGF1_SHA1, sourceData);
                break;
            case "SHA-224":
                params = new graphene__namespace.RsaOaepParams(graphene__namespace.MechanismEnum.SHA224, graphene__namespace.RsaMgf.MGF1_SHA224, sourceData);
                break;
            case "SHA-256":
                params = new graphene__namespace.RsaOaepParams(graphene__namespace.MechanismEnum.SHA256, graphene__namespace.RsaMgf.MGF1_SHA256, sourceData);
                break;
            case "SHA-384":
                params = new graphene__namespace.RsaOaepParams(graphene__namespace.MechanismEnum.SHA384, graphene__namespace.RsaMgf.MGF1_SHA384, sourceData);
                break;
            case "SHA-512":
                params = new graphene__namespace.RsaOaepParams(graphene__namespace.MechanismEnum.SHA512, graphene__namespace.RsaMgf.MGF1_SHA512, sourceData);
                break;
            default:
                throw new core__namespace.OperationError(`Cannot create PKCS11 mechanism from algorithm '${keyAlg.hash.name}'`);
        }
    ...
    mgf1(algorithm, seed, length = 0) {
        ...
        for (let i = 0; i < chunks; i++) {
            ...
            let chunk = crypto__namespace.createHash(algorithm.name.replace("-", ""))
                .update(seed)
                .update(counter)
                .digest();
...
class RsaEsProvider extends core__namespace.RsaProvider {
    constructor(container) {
        super();
        this.container = container;
...
class ShakeCrypto {
    static digest(algorithm, data) {
        const hash = crypto__namespace.createHash(algorithm.name.toLowerCase(), { outputLength: algorithm.length })
            .update(Buffer.from(data)).digest();
        return pvtsutils__namespace.BufferSourceConverter.toArrayBuffer(hash);
    }
}
...
const OBJECT_TYPES = [graphene__namespace.ObjectClass.PRIVATE_KEY, graphene__namespace.ObjectClass.PUBLIC_KEY, graphene__namespace.ObjectClass.SECRET_KEY];
class KeyStorage {
    constructor(crypto) {
        this.crypto = crypto;
    }
    ...
    async getItem(key, ...args) {
        ...
        if (subjectObject) {
            ...
            if (algorithm) {
                ...
                switch (p11Key.type) {
                    case graphene__namespace.KeyType.RSA: {
                    ...
                    case graphene__namespace.KeyType.EC: {
                        if (p11Key.sign || p11Key.verify) {
                            alg.name = "ECDSA";
                        }
                        else {
                    ...
                    case graphene__namespace.KeyType.GENERIC_SECRET:
                    case graphene__namespace.KeyType.AES: {
                        if (p11Key.sign || p11Key.verify) {
                            alg.name = "HMAC";
                        }
                        else {
                    ...
                    default:
                        throw new Error(`Unsupported type of key '${graphene__namespace.KeyType[p11Key.type] || p11Key.type}'`);
                }
            }
            let CryptoKeyClass;
    ...
    async setItem(data) {
        if (!(data instanceof CryptoKey)) {
            throw new core__namespace.CryptoError("Parameter 1 is not P11CryptoKey");
        }
        const p11Key = data;
...
class TemplateBuilder {
    build(params) {
        ...
        if (action === "copy") {
            ...
            if (attributes.id) {
                template.id = Buffer.from(pvtsutils__namespace.BufferSourceConverter.toArrayBuffer(attributes.id));
            }
            ...
            switch (type) {
                case "private":
                    Object.assign(template, {
                        class: graphene__namespace.ObjectClass.PRIVATE_KEY,
                ...
                case "public":
                    Object.assign(template, {
                        token: !!attributes.token,
                        class: graphene__namespace.ObjectClass.PUBLIC_KEY,
                        private: false,
                        derive,
                        verify,
                        encrypt,
                ...
                case "request":
                    ...
                    Object.assign(template, {
                        class: graphene__namespace.ObjectClass.DATA,
                ...
                case "x509":
                    Object.assign(template, {
                        class: graphene__namespace.ObjectClass.CERTIFICATE,
                        certType: graphene__namespace.CertificateType.X_509,
                        private: false,
                    });
                    break;
            }
...
var _Crypto_session;
const WebCryptoError = core__namespace.CryptoError;
class Crypto extends core__namespace.Crypto {
    get session() {
        Assert.isSession(tslib.__classPrivateFieldGet(this, _Crypto_session, "f"));
        return tslib.__classPrivateFieldGet(this, _Crypto_session, "f");
    }
    constructor(props) {
        super();
        _Crypto_session.set(this, undefined);
        this.templateBuilder = new TemplateBuilder();
        const mod = graphene__namespace.Module.load(props.library, props.name || props.library);
        ...
        try {
            ...
            if (!(e instanceof pkcs11__namespace.Pkcs11Error && e.code === pkcs11__namespace.CKR_CRYPTOKI_ALREADY_INITIALIZED)) {
                throw e;
            }
        }
        ...
        this.isLoginRequired = !!(this.token.flags & graphene__namespace.TokenFlag.LOGIN_REQUIRED);
        ...
        for (const i in props.vendors) {
            graphene__namespace.Mechanism.vendor(props.vendors[i]);
        }
    ...
    open(rw) {
        let flags = graphene__namespace.SessionFlag.SERIAL_SESSION;
        if (rw) {
            flags |= graphene__namespace.SessionFlag.RW_SESSION;
        }
        tslib.__classPrivateFieldSet(this, _Crypto_session, this.slot.open(flags), "f");
    ...
    login(pin) {
        ...
        try {
            ...
            if (!(error instanceof pkcs11__namespace.Pkcs11Error && error.code === pkcs11__namespace.CKR_USER_ALREADY_LOGGED_IN)) {
                throw error;
            }
        }
    ...
    logout() {
        ...
        try {
            ...
            if (!(error instanceof pkcs11__namespace.Pkcs11Error && error.code === pkcs11__namespace.CKR_USER_NOT_LOGGED_IN)) {
                throw error;
            }
        }
    ...
    getRandomValues(array) {
        ...
        if (array.byteLength > 65536) {
            throw new core__namespace.CryptoError(`Failed to execute 'getRandomValues' on 'Crypto': The ArrayBufferView's byte length (${array.byteLength}) exceeds the number of bytes of entropy available via this API (65536).`);
        }
